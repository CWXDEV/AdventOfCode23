    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var line in lines)
            {
                ...
                var rounds = game[1].Split( new string[] { ";" }, StringSplitOptions.None);
                
                for (int i = 0; i < rounds.Length; i++)
                {
                    var colours = rounds[i].Split(new string[] { "," }, StringSplitOptions.None);

                    foreach (var colour in colours)
                    {
                        var result = 0;
                        var check = int.TryParse($"{colour.Trim().Split(new string[] { " " }, StringSplitOptions.None)[0]}", out result);

                        Console.WriteLine($"check int: {result}");

                        if (colour.Contains("red") && check)
                        {
                            if (result > maxRed)
                            {
                                toAdd = false;
                        ...
                        if (colour.Contains("green") && check)
                        {
                            if (result > maxGreen)
                            {
                                toAdd = false;
                ...
                if (int.TryParse($"{game[0].Split(new string[] { " " }, StringSplitOptions.None)[1]}", out int resultresult) && toAdd)
                {
                    resultOfAllStrings += resultresult;
                }
            }

            Console.WriteLine($"result: {resultOfAllStrings}");
        }
...
game 3 would have been impossible because at one point the Elf showed you 20 red cubes at once; similarly, game 4 would also have been impossible because the Elf showed you 15 blue cubes at once. 
