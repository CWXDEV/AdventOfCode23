    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                var youtNumberString = game.Split(" | ")[1];

                var winningNumbersSplit = winningNumbersString.Trim().Split(" ");
                var yourNumbersSplit = youtNumberString.Trim().Split(" ");

                foreach (var number in winningNumbersSplit)
                {
                ...
                foreach (var number in yourNumbersSplit)
                {
                    if (int.TryParse($"{number}", out int result2))
                    {
                        numbersToCheck.Add(result2);
                    }
                    else
                    {
                        throw new Exception("something fucked up 2");
                    }
                }
...
I bet the gardener would know. He's on a different island, though - er, the small kind surrounded by water, not the floating kind. We really need to come up with a better naming scheme. 
...
In the above example, card 1 has five winning numbers (41, 48, 83, 86, and 17) and eight numbers you have (83, 86, 6, 31, 17, 9, 48, and 53). Of the numbers you have, 
four of them (48, 83, 17, and 86) are winning numbers! That means card 1 is worth 8 points (1 for the first match, then doubled three times for each of the three matches after the first).
